...
if(isset($_REQUEST['approve'])) {
    ...
    $sql="SELECT * FROM req_visacard WHERE id='$id'";
    $result=  mysql_query($sql) or die(mysql_error());
    $rws=  mysql_fetch_array($result);
                
    if($rws[3]=="PENDING")
    $sql="UPDATE req_visacard SET visacard_status='ISSUED' WHERE id='$id'";
    mysql_query($sql) or die(mysql_error());
	header("location:requests?visacard=1");
...
// Do these steps when card request is denied (by staff)
    ...
    if($rws[3]=="PENDING")
    $sql="UPDATE req_visacard SET visacard_status='DENIED' WHERE id='$id'";
    mysql_query($sql) or die(mysql_error());
...
// Do these steps when card request is deleted (by staff)
    ...
    $rws=  mysql_fetch_array($result);         
    if($rws[3]=="PENDING")
	$sql="DELETE FROM req_visacard WHERE id='$id'";    
    mysql_query($sql) or die(mysql_error());
